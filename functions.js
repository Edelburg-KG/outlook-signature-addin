...
// whenever someone starts composing — no task pane or button involved. It
// inserts the mailbox's standard signature, <domain>/<local>.html; choosing
// another one is the task pane's job (taskpane.html / taskpane.js).
//
...
// Signatures live under a folder per email domain, e.g.
// example.com/j.doe.html, so one bucket serves every tenant and
// mailboxes that share a local part (j.doe@example.com, j.doe@example.org)
// don't collide. Returns "<domain>/<local>", URL-encoded, or null for
// anything that isn't a plain address. taskpane.js has the same function.
...
function checkSignature(eventObj) {
  ...
  console.log("[Edelburg Signature] signed-in email:", email, "signature path:", path);

  if (!path) {
    console.log("[Edelburg Signature] not a usable address, skipping");
    eventObj.completed();
    return;
  }

  const url = SIGNATURE_BASE_URL + "/" + path + ".html";
  ...
  fetch(url, { cache: "no-store" })
    ...
    .catch(function (error) {
      ...
      // yet in signatures.csv), or the fetch failed — leave the compose
